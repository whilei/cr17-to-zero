...
Without learning, Clojure will not grow. Which is almost a syllogism; learning _is_ growth.

It's a funny thing for someone to be an advocate of language, and yet, so common in the land of computational linguists.
https://wiki.theory.org/YourLanguageSucks

Perhaps the reason for it that in this realm, language is also a technology. And it's a technology that - at least in some part - suffers from many of the same weaknesses that its designs do; among them, the _Nightclub problem_, where usefulness and use are circularly dependent. Twitter is easy - getting users and keeping them is the weakness, and strength.
